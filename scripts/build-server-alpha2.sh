#!/usr/bin/env bash
# Build and validate the Lyra OS Server Alpha 2 ISO from a real terminal.
...
if [ "$(./scripts/server-release.py field version_id)" != "2026.08-alpha2" ]; then
  echo "ERRO: release-server.toml não aponta para 2026.08-alpha2." >&2
  exit 1
fi
